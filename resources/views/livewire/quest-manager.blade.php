<div>
    ...
    <div class="quest-manager-container integrated-page-container">
        <!-- Alert Toast -->
        ...
        <div class="page-header-mystic">
            <div class="title-section">
                <h1 class="spell-title-main">Missões de Hogsmead</h1>
            ...
            <div class="header-controls">
                ...
                <button wire:click="openModal" class="btn-magical-seal">
                    ...
                    <div class="seal-inner">
                        ...
                        <span class="seal-text">NOVA MISSÃO</span>
                    </div>
                </button>
            </div>
        </div>
            ...
            <div class="spell-card-wrapper">
                <div class="spell-card-organic">
                    ...
                    <div class="spell-visual-organic">
                        @if($quest->image)
                            <img src="{{ asset('storage/' . $quest->image->path) }}" alt="{{ $quest->name }}" class="spell-image-organic">
                        @else
                    ...
                    <div class="card-content-organic">
                        <h3 class="spell-name-organic">{{ $quest->name }}</h3>
                        ...
                        <p class="spell-action-organic">{{ Illuminate\Support\Str::limit($quest->description, 80) ?? 'Sem descrição definida.' }}</p>

                        <div class="spell-stats-organic">
                            <div class="stat-item">
                                <span class="stat-label">Ganho</span>
                                <span class="stat-value">{{ $quest->gain }} XP</span>
                            </div>
                        </div>

                        <div class="wheel-association">
                            🎡 {{ $quest->wheel?->name ?? 'Sem Roda' }}
                        </div>
                    </div>
                </div>
                <!-- Quick Edit Button -->
                <button wire:click="edit({{ $quest->id }})" class="quick-edit-btn" title="Editar Missão">✎</button>
                <!-- Quick Delete Button -->
                <button wire:click="delete({{ $quest->id }})" wire:confirm="Tem certeza que deseja excluir esta missão?" class="quick-delete-btn" title="Excluir Missão">🗑️</button>
            </div>
        @empty
            <div class="empty-state-mystic">
                <div class="empty-icon-parchment">📭</div>
                <p class="empty-msg">Nenhuma missão encontrada. O grimório aguarda suas Missões.</p>
                <button wire:click="openModal" class="btn-magical-seal">CONJURAR MISSÃO</button>
            </div>
        @endforelse
        </div>
        ...
        @if($quests->hasPages())
            <div class="pagination-container-mystic">
                {{ $quests->links('livewire.mystic-pagination') }}
            </div>
        @endif
            ...
            <div class="modal-parchment-spell" wire:click.stop>
                ...
                <div class="modal-header-mystic">
                    <div class="header-content-wrapper">
                        <h2>{{ $isEditing ? 'Reescrever Missão' : 'Conjurar Nova Missão' }}</h2>
                    </div>
                ...
                <form wire:submit.prevent="save" class="modal-form-full">
                    <div class="modal-body-mystic scrollable-content">
                        ...
                        <div class="form-group-parchment">
                            ...
                            <input type="text" id="quest_name" wire:model="name" class="input-parchment" placeholder="Ex: Derrotar o Dragão">
                            @error('name') <span class="error-magical">{{ $message }}</span> @enderror
                        </div>

                        <div class="form-group-parchment">
                            <label for="quest_description">Descrição</label>
                            <textarea id="quest_description" wire:model="description" class="input-parchment textarea-compact" placeholder="O que deve ser feito na missão..."></textarea>
                            @error('description') <span class="error-magical">{{ $message }}</span> @enderror
                        </div>

                        <div class="form-group-parchment">
                            <label for="quest_gain">Ganho (XP)</label>
                            <input type="number" id="quest_gain" wire:model="gain" class="input-parchment" placeholder="0">
                        ...
                        <div class="form-group-parchment">
                            ...
                            <input type="file" id="quest_image" wire:model="image" accept="image/*" class="input-parchment" style="border: none; padding: 0.5rem 0;">
                    ...
                    <div class="modal-footer-mystic compact-footer">
                        ...
                        <button type="submit" class="btn-magical-seal">
                            <div class="seal-glow"></div>
                            <div class="seal-inner">
                                <span class="seal-text">{{ $isEditing ? 'SALVAR ALTERAÇÕES' : 'CONJURAR MISSÃO' }}</span>
                            </div>
                        </button>
                    </div>
                </form>
